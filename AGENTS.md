...
- 太字記法(`**hoge**`)は使用しない
- 強調が必要な場合はコロン区切り（例: `項目名: 説明`）や見出しを使用
...
Claude Code、Codex CLI、Gemini CLI は相互補完的に活用する。

| AI              | 強み                                         | 適した場面                                     |
| --------------- | -------------------------------------------- | ---------------------------------------------- |
| **Claude Code** | 発想力・創造性・対話的探索                   | 設計構想、新しいアイデア、曖昧な要件の整理     |
| **Codex CLI**   | 実行力・確実性・タスク完遂                   | やることが決まった実装、修正、リファクタリング |
| **Gemini CLI**  | 調査能力・大規模コンテキスト・マルチモーダル | リサーチ、ライブラリ調査、PDF/動画分析         |

**委譲の判断基準**:

- 発想・創造・対話 → Claude Code （`claude-system`スキル参照）
- 実行・修正・リファクタ → Codex CLI（`codex-system`スキル参照）
- 調査・分析・マルチモーダル → Gemini CLI（`gemini-system`スキル参照）
- ただし、claudeであればclaude-system, codexであればcodex-systemを参照しない
...
- 追加コミット優先: Force pushではなく追加コミットで修正する
- 理由: Squash mergeを前提としているため、履歴が増えても最終的に1コミットに集約される
- 安全性を優先し、レビュー履歴を保持する
